...
int lazy_query(int *tree, int ss, int se, int qs, int qe, int index)
{
    ...
    // Partial overlap
    int m = ss + ((se - ss) / 2);
    int left = lazy_query(tree, ss, m, qs, qe, 2 * index);
    int right = lazy_query(tree, m + 1, se, qs, qe, 2 * index + 1);
...
void solve()
{
    ...
    lazy_range_update(tree, 0, n - 1, 0, 2, 1, +10);
    lazy_range_update(tree, 0, n - 1, 0, 4, 1, +10);

    cout << lazy_query(tree, 0, n - 1, 1, 1, 1) << endl;

    lazy_range_update(tree, 0, n - 1, 3, 4, 1, +10);

    cout << lazy_query(tree, 0, n - 1, 3, 5, 1) << endl;
}
